...
async def summarize_route(payload: SummarizeRequest):
    ...
    if mode == "compare":
        res = {
            "extractive": summarize(payload.text, mode="extractive", length=payload.length, num_sentences=payload.num_sentences),
            "abstractive": summarize(payload.text, mode="abstractive", length=payload.length, num_sentences=payload.num_sentences),
        }
    else:
        res = summarize(payload.text, mode=mode, length=payload.length, num_sentences=payload.num_sentences)
